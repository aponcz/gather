...
RSpec.describe 'Loan add contacts', type: :request do
  ...
  let!(:loan_contact) { LoanContact.create!(loan: loan, contact: contact_one) }
  ...
  describe 'POST /api/v1/loans/:id/add_contacts' do
    ...
    it 'adds non-global recipients directly to an existing loan' do
      ...
      body = json_body
      expect(body['added_contact_count']).to eq(1)
      expect(loan.reload.loan_contacts.pluck(:email).any? { |email| email.start_with?('extra-') }).to eq(true)
    end
