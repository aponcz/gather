...
RSpec.describe 'Invite import loans', type: :request do
  ...
  describe 'POST /api/v1/invites/import_loans' do
    it 'imports loans from protext and creates invites' do
      ...
      post '/api/v1/invites/import_loans', headers: headers
      ...
      invites = company.invites.order(:created_at)
      expect(invites.count).to eq(2)
      expect(invites.first.title).to eq('Jane Borrower')
      expect(invites.first.message).to eq('OAuth imported from ProText loan sync.')
      expect(invites.first.protext_id).to be_nil

      recipients = invites.first.invite_contacts
      expect(recipients.count).to eq(0)
    end

    it 'skips duplicate loans already imported' do
      existing_invite = company.invites.create!(
        title: 'Existing Loan Invite',
        message: "Imported from ProText loan sync.\n[ProText Loan ID: loan-1]",
        created_by: user,
        status: 'draft',
        protext_id: 'loan-1'
      )
      existing_invite.invite_contacts.create!(name: 'Existing Borrower', email: 'existing.borrower@example.test')
      ...
      expect(body['created_count']).to eq(2)
      expect(body['skipped_count']).to eq(0)
      expect(company.invites.count).to eq(3)
    end
